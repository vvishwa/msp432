...
// copy 512 bytes from ROM (disk) into RAM (buff)
		...
		uint8_t *ptROM;
		uint32_t startADDRESS = EDISK_ADDR_MIN + 512*sector;
		ptROM = (uint8_t*)startADDRESS;
		if (startADDRESS <= EDISK_ADDR_MAX){
			for (int i = 0; i < 512; i++){
				*buff=*ptROM;
				ptROM++;
				buff++;
...
// you can use Flash_FastWrite or Flash_WriteArray
		...
		if (startADDRESS > EDISK_ADDR_MAX)
				...
				writtenCount = Flash_WriteArray((uint32_t *)buff, startADDRESS, 512/4);
    
		if (writtenCount == 512*sector/32)
			return RES_OK;
		else
			return RES_ERROR;
}
