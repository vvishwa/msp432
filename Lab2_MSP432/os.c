...
uint32_t periodTask1;
uint32_t periodTask2;
...
void OS_Wait(int32_t *semaPt){
	DisableInterrupts();
	while((*semaPt) == 0) {
		EnableInterrupts();//interrupt can occur here
		DisableInterrupts();
	}
	(*semaPt) =(*semaPt) - 1;
	EnableInterrupts();
...
int32_t Send; // semaphore tells consumer whether there is data. 0 no data, 1 new data
int32_t Ack; // semaphore consumer signals to producer if data has been read. 0 data not read, 1 data read
