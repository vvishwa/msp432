...
funcptr_f pthread[NUMPERIODIC];
uint32_t ptperiod[NUMPERIODIC];
uint32_t whichPeriodThread;
uint32_t Counter[NUMPERIODIC];
...
// **RUN PERIODIC THREADS, DECREMENT SLEEP COUNTERS
	...
	for(int i=0; i<NUMPERIODIC; i++) {
		Counter[i]++;
		if(Counter[i] == ptperiod[i]){
			(*pthread[i])(); // runs periodically
			Counter[i] = 0;
		}
	}	
	...
	/*
	for(int j=0;j<total_pcb;j++){ // periodic threads scanned first
		if (pcbs[j].periodtask_current == 1){
			//pcbs[j].periodictask();
			pcbs[j].periodtask_current=pcbs[j].periodtask_period;			
		}else{
			pcbs[j].periodtask_current--;
		}
	}
...
// like setting up periodic timer to run runperiodicevents
  whichPeriodThread = 0;
	for(int i = 0; i< NUMPERIODIC; i++)
		Counter[i]=0;
	BSP_PeriodicTask_InitB(&runperiodicevents, 1000, 2);	// Periodic event 
}
...
struct pcb{
	void(*periodictask)();
	uint32_t periodtask_period;
	uint32_t periodtask_current;
};
typedef struct pcb pcbType;
pcbType pcbs[NUMPERIODIC];
uint32_t total_pcb=0;
...
int OS_AddPeriodicEventThread(void(*thread)(void), uint32_t period){
	...
	if(whichPeriodThread == NUMPERIODIC)
 return 0;
 
 int32_t status;
 status = StartCritical();
 pthread[whichPeriodThread] = thread;
 ptperiod[whichPeriodThread] = period;
 whichPeriodThread++;
 EndCritical(status);
 return 1;
	...
	/*
	if(total_pcb==NUMPERIODIC){
		return 0;
	}else{
		pcbs[total_pcb].periodictask = thread;
		pcbs[total_pcb].periodtask_period = period;
		pcbs[total_pcb].periodtask_current = 1;
		BSP_PeriodicTask_InitB(thread, period, 2);
		total_pcb++;
		return 1;
	}
	*/
...
// suspend, stops running
	...
	//BSP_PeriodicTask_Init(&runperiodicevents,1000/sleepTime,2);
	OS_Suspend();
}
